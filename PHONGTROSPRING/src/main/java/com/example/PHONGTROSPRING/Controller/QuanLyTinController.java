...
public class QuanLyTinController {
	...
	public String update(RedirectAttributes redirectattributes ,Model model, @PathVariable int id) {
		...
		if(listings.getExpiryDate().isAfter(LocalDateTime.now())) {
			System.out.println("coqua");
			...
			redirectattributes.addFlashAttribute("errorMessage", "Tin đã hết hạn không thể đăng sửa");
			return "redirect:/quanlytin";
		}	
	}
	...
	@PostMapping("/quanlytin/giahan/{id}")
	@ResponseBody
	public ResponseEntity<?> giahanExcute(HttpSession session, @RequestBody RequestThanhToan rq, @PathVariable int id) {
	    User user = (User) session.getAttribute("user");
	    if (quanLyTinService.giahantin(rq, id, user)) {
	        return ResponseEntity.ok().body(Map.of("message", "Gia hạn thành công"));
	    } else {
	        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", "Gia hạn thất bại"));
	    }
	}
	
	@GetMapping("/quanlytin/delete/{id}")
	public String deleteExcute(@PathVariable int id,RedirectAttributes redirectattributes) {
		if(quanLyTinService.deleteTin(id)) {
