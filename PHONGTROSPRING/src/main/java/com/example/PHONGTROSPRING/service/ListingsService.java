...
public class ListingsService {
	...
	public Page<ListingsResponse> getListingsByDistrictAndOrder(int roomtype_id, int city_id, int district_id, // Thay
																												// đổi
																												// tham
																												// số
																												// thành
		...
		Pageable pageable = PageRequest.of(page, size);
		return listingRepository.findByDistrictAndOrderBy(roomtype_id, city_id, district_id, // Truyền district_id
				orderby, pageable);
	}
	...
	public List<Listings> getListingOnMap(int room, List<String> features, String price, String area) {
		// Xử lý giá trị price thành minPrice và maxPrice
		String[] priceRange = price != null ? price.split("_") : null;
		String minPrice = priceRange[0];
		String maxPrice = priceRange[1];

		// Xử lý area thành minArea và maxArea
		String[] areaRange = area != null ? area.split("_") : null;
		String minArea = areaRange[0];
		String maxArea = areaRange[1];

		// Nếu features là null hoặc rỗng, gán tất cả các boolean thành null (để không
		// lọc theo features)
		if (features == null || features.isEmpty()) {
			return listingRepository.findListingsWithFilters(room, null, null, null, null, null, null, null, null, null,
					null, null, minPrice, maxPrice, minArea, maxArea);
		}

		// Chuyển features thành các boolean tương ứng nếu features không phải là null
		Boolean fullFurniture = features.contains("fullFurniture") ? true : null;
		Boolean washingMachine = features.contains("washingMachine") ? true : null;
		Boolean freeTime = features.contains("freeTime") ? true : null;
		Boolean attic = features.contains("attic") ? true : null;
		Boolean refrigerator = features.contains("refrigerator") ? true : null;
		Boolean protection = features.contains("protection") ? true : null;
		Boolean kitchenShelves = features.contains("kitchenShelves") ? true : null;
		Boolean elevator = features.contains("elevator") ? true : null;
		Boolean parkingBasement = features.contains("parkingBasement") ? true : null;
		Boolean airConditioning = features.contains("airConditioning") ? true : null;
		Boolean commonOwner = features.contains("commonOwner") ? true : null;

		// Gọi repository với các tham số
		return listingRepository.findListingsWithFilters(room, fullFurniture, washingMachine, freeTime, attic,
				refrigerator, protection, kitchenShelves, elevator, parkingBasement, airConditioning, commonOwner,
