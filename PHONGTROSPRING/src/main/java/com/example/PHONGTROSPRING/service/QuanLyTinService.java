...
public class QuanLyTinService {
	...
	@Autowired
	private ThongTinNguoiThueRepository thongTinNguoiThueRepository;
	
	@Autowired
	private FavoritePageRepository favoritePageRepository;
	...
	public boolean deleteTin(int id) {
		Listings listingdelete = ListingsRepository.findById(id).get();

		
		if(favoritePageRepository.findByListing(listingdelete) !=null) {
			favoritePageRepository.deleteFavoritebylisting(listingdelete);
		}
		if(!phongTroRepository.findByListings(listingdelete).isEmpty()) {
			thongTinNguoiThueRepository.deleteByPhongTros(phongTroRepository.findByListings(listingdelete));
			phongTroRepository.deleteAll(phongTroRepository.findByListings(listingdelete));
		}
		
		if(ImagesRepository.findByListingItemId(id).size()>0) {
			ImagesRepository.deleteAll(ImagesRepository.findByListingItemId(id));
		}
		if(paymentHistoryRepository.findByListings(listingdelete).size()>0) {
			paymentHistoryRepository.deleteAll(paymentHistoryRepository.findByListings(listingdelete));
		}
		if(ListingsFeaturesRepository.findByListings(listingdelete) != null) {
			ListingsFeaturesRepository.delete(ListingsFeaturesRepository.findByListings(listingdelete));
		}
		
		ListingsRepository.deleteById(id);
		if(ListingsRepository.findById(id).isEmpty()) {
			return true;
		}else
			return false;
	}
