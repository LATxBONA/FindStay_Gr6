...
//	
	...
	@Query("SELECT l FROM Listings l " + "LEFT JOIN ListingsFeatures lf ON l.itemId = lf.listings.itemId "
			+ "WHERE (:room = 0 OR l.roomType.id = :room) "
			+ "AND (:fullFurniture IS NULL OR lf.fullFurniture = :fullFurniture) "
			+ "AND (:washingMachine IS NULL OR lf.washingMachine = :washingMachine) "
			+ "AND (:freeTime IS NULL OR lf.freeTime = :freeTime) " + "AND (:attic IS NULL OR lf.attic = :attic) "
			+ "AND (:refrigerator IS NULL OR lf.refrigerator = :refrigerator) "
			+ "AND (:protection IS NULL OR lf.protection = :protection) "
			+ "AND (:kitchenShelves IS NULL OR lf.kitchenShelves = :kitchenShelves) "
			+ "AND (:elevator IS NULL OR lf.elevator = :elevator) "
			+ "AND (:parkingBasement IS NULL OR lf.parkingBasement = :parkingBasement) "
			+ "AND (:airConditioning IS NULL OR lf.airConditioning = :airConditioning) "
			+ "AND (:commonOwner IS NULL OR lf.commonOwner = :commonOwner) "
			+ "AND (:minPrice IS NULL OR l.price >= :minPrice) " + "AND (:maxPrice IS NULL OR l.price <= :maxPrice) "
			+ "AND (:minArea IS NULL OR l.area >= :minArea) "
			+ "AND (:maxArea IS NULL OR l.area <= :maxArea) AND l.status = 'Đã duyệt'")
	List<Listings> findListingsWithFilters(@Param("room") Integer room, @Param("fullFurniture") Boolean fullFurniture,
			@Param("washingMachine") Boolean washingMachine, @Param("freeTime") Boolean freeTime,
			@Param("attic") Boolean attic, @Param("refrigerator") Boolean refrigerator,
			@Param("protection") Boolean protection, @Param("kitchenShelves") Boolean kitchenShelves,
			@Param("elevator") Boolean elevator, @Param("parkingBasement") Boolean parkingBasement,
			@Param("airConditioning") Boolean airConditioning, @Param("commonOwner") Boolean commonOwner,
			@Param("minPrice") String minPrice, @Param("maxPrice") String maxPrice, @Param("minArea") String minArea,
